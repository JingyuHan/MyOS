...
void console_task(struct SHEET *sheet, int memtotal)
{
	...
	task->fat = fat;
	if (nihongo[4096] != 0xff) {	/* 是否载入了全角字符 */
		task->langmode = 1;
	} else {
		task->langmode = 0;
	}
	...
	for (;;) {
		...
		if (fifo32_status(&task->fifo) == 0) {
			...
			if (i == 3) {	/* 光标OFF */
				if (cons.sht != 0) {
					boxfill8(cons.sht->buf, cons.sht->bxsize, 0, cons.cur_x, cons.cur_y, cons.cur_x + 7, cons.cur_y + 15);
				}
				cons.cur_c = -1;
			}
			if (i == 4) {	/* 点击命令窗口的「×」 */
...
{
	//struct SHTCTL *shtctl = (struct SHTCTL *) *((int *) 0x0fe4);
	//struct SHEET *sht0 = shtctl->sheets[1];
	//struct TASK *task0 = sht0->task;
	//struct CONSOLE *cons0 =  task0->cons;
	//cons_putstr0(cons0, cmdline);
