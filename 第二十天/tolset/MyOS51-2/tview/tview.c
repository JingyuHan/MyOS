...
int strtol(char *s, char **endp, int base);	/* ?èyÅistdlib.hÅj */
...
err:
	...
	*r = 0;
	...
	txtbuf[0] = 0x0a; /* ?ï∫ópìI?çsë„? */
	api_fread(txtbuf + 1, j, i);
	api_fclose(i);
	txtbuf[j + 1] = 0;
	q = txtbuf + 1;
	for (p = txtbuf + 1; *p != 0; p++) {	/* ?óπ??óù?ìæ??ÅC?ù{0x0dìIë„? */
		if (*p != 0x0d) {
			*q = *p;
			q++;
		}
	...
	for (;;) {
		...
		if (i == '4') {
			for (;;) {
				...
				if (api_getkey(0) != '4') { /* Ç‡Ç§'4'î@â ñvóLà¬â∫Åg4Åh,??óù?ë© */
					break;
				}
			}
		}
		...
		if (i == '8') {
			for (;;) {
				for (j = 0; j < spd_y; j++) {
					...
					for (p--; p[-1] != 0x0a; p--) { } /* âÒìûè„àÍò¢éöïÑ?0x0a?é~ */
				}
				if (api_getkey(0) != '8') {
					break;
				}
