...
int strtol(char *s, char **endp, int base);	/* ?èyîüêîÅistdlib.hÅj */
...
void HariMain(void)
{
	...
	/* âπçÜó^?ó¶ÅiMHZÅj?è∆ï\ */
	/* î@ÅFO4A?440Hz ë¶440000 */
	/* ëÊ16î™ìxìIA?1802240Hzë¶1802240000 */
	/* à»â∫?ëÊ16î™ìxìIóÒï\(CÅ`B) */
...
file_error:
	...
	r = txtbuf;
	i = 0; /* í èÌñÕéÆ */
	for (p = txtbuf; *p != 0; p++) {	/* ?ï˚ï÷?óùÅCè´íç?òaãÛîí?ãé */
		if (i == 0 && *p > ' ') {	/* ïsê•ãÛäiàΩ?çsïÑ */
			if (*p == '/') {
				if (p[1] == '*') {
					i = 1;
				} else if (p[1] == '/') {
		...
		} else if (i == 1 && *p == '*' && p[1] == '/') {	/* íiíç? */
			p++;
			i = 0;
		} else if (i == 2 && *p == 0x0a) {	/* çsíç? */
			i = 0;
		} else if (i == 3) {	/* éöïÑã¯ */
			*r = *p;
			r++;
	...
	*r = 0;
	...
	for (;;) {
		if (('A' <= *p && *p <= 'G') || *p == 'R') {	/* âπïÑÅCãxé~ïÑ */
			...
			p++;
			o++;
		} else if (*p == 'O') {	/* î™ìxéwíË */
			o = strtol(p + 1, &p, 10);
		} else if (*p == 'Q') {	/* QéQêîéwíË */
			q = strtol(p + 1, &p, 10);
		} else if (*p == 'L') { /* ‡“?âπ?éwíË */ 
			l = strtol(p + 1, &p, 10);
			if (l == 0) {
