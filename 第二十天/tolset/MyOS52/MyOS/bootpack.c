...
void HariMain(void)
{
	...
	*((int *) 0x0fe4) = (int) shtctl;
	...
	if (finfo != 0) {
		...
		for (i = 0; i < 16 * 256; i++) {
			nihongo[i] = hankaku[i]; /* 没有字库，半角复制英文字库 */
		}
		for (i = 16 * 256; i < 16 * 256 + 32 * 94 * 47; i++) {
			nihongo[i] = 0xff; /* 没有字库，全角用0xff填充 */
		}
	}
	...
	while(1){
		...
		if (fifo32_status(&fifo) == 0) {
			...
			if (256 <= i && i <= 511) {/*键盘*/
				...
				if (i == 256 + 0x3c && key_ctrl != 0) {	/* Ctrl+F2 */
					/* 自动将焦点切换到新打开的命令行窗口 */
					if (key_win != 0) {
						keywin_off(key_win);
					}
								...
								if (0 <= x && x < sht->bxsize && 0 <= y && y < sht->bysize) {
									if (sht->buf[y * sht->bxsize + x] != sht->col_inv) {
										...
										if (sht->bxsize - 21 <= x && x < sht->bxsize - 5 && 5 <= y && y < 19) {
											/* 点击「×」 */
											if ((sht->flags & 0x10) != 0) {		/* 是否是由应用程序产生的窗口？ */
												task = sht->task;
												
												cons_putstr0(task->cons, "\nBreak(mouse) :\n");
												io_cli();	/* 强制结束任务时，禁止任务切换 */
												task->tss.eax = (int) &(task->tss.esp0);
												task->tss.eip = (int) asm_end_app;
												io_sti();
												task_run(task, -1, 0);
...
struct SHEET *open_console(struct SHTCTL *shtctl, unsigned int memtotal)
{
	...
	unsigned char *buf = (unsigned char *) memman_alloc_4k(memman, 356 * 365);
	sheet_setbuf(sht, buf, 356, 365, -1); 
	make_window(buf, 356, 365, "console", 0);
	make_textbox8(sht, 8, 28, 340, 328, 0);
